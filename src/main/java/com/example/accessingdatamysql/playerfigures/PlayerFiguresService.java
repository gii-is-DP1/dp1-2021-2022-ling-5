...
public class PlayerFiguresService {
    ...
    @Transactional
    public PlayerFigures savePlayerFigures(PlayerFigures playerFigures) throws DataAccessException {
    ...
    public List<PlayerFigures> findAllPlayerFigures() {
        return StreamSupport.stream(playerFiguresRepository.findAll().spliterator(), false).collect(Collectors.toList());
    }

    public List<PlayerFigures> findAllPlayerFiguresByPlayer(Long playerId) {
        return StreamSupport.stream(playerFiguresRepository.findAll().spliterator(), false)
    ...
    public List<PlayerFigures> findAllPlayerFiguresByFigure(Long figureId) {
        return StreamSupport.stream(playerFiguresRepository.findAll().spliterator(), false)
                .filter(result -> result.getFigure().getId() == figureId).collect(Collectors.toList());
    }
    ...
    public void deleteAllPlayerFiguresByPlayer(Long playerId) {
        findAllPlayerFiguresByPlayer(playerId).stream().forEach(result -> playerFiguresRepository.deleteById(result.getId()));
    }

    public PlayerFigures getByData(Long figureId,Long playerId){
        return StreamSupport.stream(playerFiguresRepository.findAll().spliterator(), false)
                .filter(result -> result.getFigure().getId() == figureId)
                .filter(result -> result.getPlayer().getId() == playerId).collect(Collectors.toList()).get(0);
    }
